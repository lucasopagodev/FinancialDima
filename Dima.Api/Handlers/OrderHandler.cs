...
public class OrderHandler(AppDbContext context, UserManager<Models.User> userManager, SignInManager<Models.User> signInManager) : IOrderHandler
{
  public async Task<Response<Order?>> ConfirmOrderAsync(ConfirmOrderRequest request)
  {
    var order = await context.Orders.FirstOrDefaultAsync(x => x.UserId == request.UserId && x.Number == request.Number);
    ...
    if (charge is null)
      return new Response<Order?>(null, 404, "Pagamento não encotrado");
